...
| 粘土拟物     | 双向柔光阴影、超柔圆角                     |
| 新野兽派     | 粗描边、硬投影、高饱和撞色                 |
| 雨夜毛玻璃   | 雨夜照片幕墙、动态雨丝、极低透明磨砂玻璃   |
| 极客赛博终端 | CRT 荧光字符、扫描线、日志格式、闪烁光标   |
| 纸质墨水屏   | 米黄纸底、素描虚线、马克笔色痕、作业本红线 |
| 复古像素风   | 8-Bit 双层像素描边、Zpix 像素字体、街机按压 |
| 拟物机械实体 | 金属拉丝、亚麻布纹、压印文字、挂历装订孔   |

> 「雨夜毛玻璃」是一张真实雨夜街景照片做幕布：`body` 背景固定铺照片
> （`src/assets/rain-night.jpg`），上面只压冷雾与暗角两层 scrim；
> `body::before` / `body::after` 是两片内联 SVG 雨丝贴图，以不同速度无缝
> 循环下落并带一点风角，`#root::after` 每隔十几秒掠过一道极淡的闪电。
> 玻璃彻底单色化：没有虹彩棱边、没有彩色光团，只有白色高光 / 描边与近黑
> 投影；面板填充不透明度压到 5%~12%，配 14~30px 的 backdrop-filter 重模糊，
> 非常透，背后的雨夜只剩一团湿润的光。浅色 = 薄雾雨夜，深色 = 更沉的夜。
> 本风格明确不考虑性能，小屏不做任何降级。
>
