...
- 周末有课自动标注「补课」；**跨午夜 / 后台恢复时自动刷新「今天」**，长时间挂着不关也不会停在昨天
...
- 任课教师一览（本学期未排课的课程自动标注）、今日课程面板、响应式布局、打印样式（自带班级/周次抬头）、`prefers-reduced-motion` 支持
- **PWA 完整闭环**：静态 Manifest + PNG（192/512/maskable）图标 + Service Worker 应用壳缓存，可安装到主屏幕、离线可用
...
| 粘土拟物     | 双向柔光阴影、超柔圆角                     |
| 新野兽派     | 粗描边、硬投影、高饱和撞色                 |
| 超清液晶玻璃 | 液态光流、边缘折射、晶体棱边与镜面高光     |
| 极客赛博终端 | CRT 荧光字符、扫描线、日志格式、闪烁光标   |
| 纸质墨水屏   | 米黄纸底、素描虚线、马克笔色痕、作业本红线 |
| 复古像素风   | 8-Bit 双层像素描边、Zpix 像素字体、街机按压 |
| 拟物机械实体 | 金属拉丝、亚麻布纹、压印文字、挂历装订孔   |

> 「超清液晶玻璃」刻意与传统毛玻璃做出区分：主模糊降到 2~3px，改用高饱和 +
> 轻微提亮 + 对比微调，背景的液态光团依然清晰可辨；每块面板内侧再用一层
> 带 `mask` 的 `backdrop-filter` 只对 1~2.5px 的边缘环带做二次采样，
> 形成玻璃厚度与透镜感，环带上叠 conic 虹彩模拟色散棱边。
> 背景分两层：`body` 上是静态液态渐变（保证任何时刻构图完整），
> `body::before` 是同一屏的流动色团图层，用 `transform` 做 38s 位移 / 旋转 / 缩放，
> GPU 合成；深色主题下叠加 `mix-blend-mode: screen`，霓虹光团像在玻璃后发光。
> 小屏自动降级：停掉流动图层、收敛模糊、去掉环带二次采样，仅保留静态虹彩棱边。
>
...
**发布新版本时请同步更新 `public/sw.js` 里的 `VERSION`**（如 `keep-v2.2.0` → `keep-v2.3.0`），
已安装客户端的旧缓存会自动清掉、拿到新版页面。
...
`body` 上的 `overscroll-behavior-y: none` 是有意为之：作为可安装 PWA，
抑制浏览器下拉刷新避免与页面内滚动冲突；若更看重网页端原生手感可改回 `auto`。
